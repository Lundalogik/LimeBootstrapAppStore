OBS! Själva servicen och Python hittar du på F:\Products\Lime CRM\Consultant Modules
------------------------------------------------------------------------------------
...
5. Skapa en virtuell miljö i mappen som du skapade i punkt 1 (kör cmd som administratör).
 * C:\VismaIntegration > c:\Python34\python.exe -m venv venv

6. Aktivera miljön
 * C:\VismaIntegration > venv\Scripts\activate.bat

7. Installera Lime CRM API klienten från PyPi
 ...
 * LimeApiUri = sökväg till Lime CRM servern (se bilder för exempel både för on-premise och hosting-sökväg)
 * LimeApiDb = namn på Lime CRM databas (skall vara tom om det är en hosting installation)
 * LimeApiUser = namn på den Lime CRM-användare som skall köra Python skriptet. Användaren skall ha skapats före installation av tjänsten
 * LimeApiPassword = lösenord för den Lime CRM-användare som kör python skriptet. Användaren skall ha skapats före installation av tjänsten
...
7. Install the Lime CRM client from PyPi
 * (venv) C:\VismaIntegration>pip install limeclient
 ...
 * LimeApiUri = path to the Lime CRM server used (see images for example for both on-premise as well as hosting paths)
 * LimeApiDb = name of the Lime CRM database connected (should be empty if hosting-installation)
 * LimeApiUser = name of the Lime CRM-user who runs the python script (as created prior to installing the service)
 * LimeApiPassword = password for the Lime CRM-user who runs the python script (as created prior to installing the service)
