/****** Object:  StoredProcedure [dbo].[csp_lip_getxmldatabase_wrapper]    Script Date: 2016-05-18 07:10:12 ******/
SET ANSI_NULLS ON
GO
...
BEGIN
	...
	SELECT @optionQuery =(
		SELECT t.name + '.' + fi.name AS [owner], CAST(data AS XML).value('(/queries/@text)[1]','nvarchar(512)') AS [text] FROM [file] f 
	...
	SET @subXml = @subxml + '</tables>' + CAST(@procedureXml AS nvarchar(MAX)) + CAST(@iconXml AS NVARCHAR(MAX)) + CAST(@optionQuery AS NVARCHAR(MAX)) + '</database>'
